<%@page import="io.github.danthe1st.poker.web.CardImageResolver"%>
<%@page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<%@taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>

<%@page import="javax.activation.ActivationDataFlavor"%>
<%@page import="io.github.danthe1st.poker.web.WebUtil"%>
<%@page import="io.github.danthe1st.poker.Game"%>
<%@page import="io.github.danthe1st.poker.Constants"%>
<%@page import="io.github.danthe1st.poker.PokerProperties"%>
		...
		<script type="text/javascript">
			...
			window.onload=function(){
				...
				InitializeTimer(${ChatProperties.getRefreshTime()});)
					...
					var wsUri = getRootUri() + "${pageContext.request.contextPath}/refresh";
							...
							<tr>
								...
								<td class="card-td">
									<img alt="${userCard}" title="${userCard }" src="${pageContext.request.contextPath}/img/${userCard}" class="minicard
										<c:if test="${(userCard== imgResolver.getCardImage(applicationScope.GAME.resultMax) )||(userCard==imgResolver.getCardImage(applicationScope.GAME.resultMin))} "> problemCard</c:if>
										<c:if test="${(userCard==imgResolver.getCardImage(applicationScope.GAME.resultAvg)) }"> goodCard</c:if>
										">
								</td>
							</tr>
						</c:forEach>
